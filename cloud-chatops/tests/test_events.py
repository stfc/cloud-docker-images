...
def test_run_global_reminder(mock_pr_reminder, mock_find_prs, mock_get_config, mock_get_token, mock_web_client):
...
@patch("events.FindPRs")
@patch("events.PRReminder")
def test_run_personal_reminder(mock_pr_reminder, mock_find_prs, mock_get_config, mock_get_token, mock_web_client):
    ...
    mock_find_prs.return_value.run.assert_called_once_with(repos=mock_repos, sort=(PRProps.CREATED_AT, False))
    ...
    mock_get_token.assert_called_once_with("SLACK_BOT_TOKEN")
    mock_pr_reminder.assert_called_once_with(mock_web_client.return_value)
    mock_pr_reminder.return_value.run.assert_called_once_with(prs=mock_find_prs.return_value.run.return_value, channel="mock_slack", filter_by=(PRProps.AUTHOR, "mock_github"), message_no_prs=False)
