...
class PRReminder:
    ...
    def run(self, prs: List[PR], channel: str, filter_by: Tuple[PRProps, Union[str, datetime, bool]] = None, message_no_prs: bool = True) -> None:
        """
        ...
        """
        if filter_by:
            prs = list(filter(lambda pr: getattr(pr, str(filter_by[0]).split(sep=".")[1].lower()) == filter_by[1], prs))

        if not prs and message_no_prs:
            self.send_message(
                text="No Pull Requests were found.",
                channel=channel
            )
            return
        ...
        reminder_message = self.send_message(text="Here are the outstanding PRs as of today:", channel=channel)
    ...
    def send_message(self, text: str, channel: str, reactions: List[str] = None, timestamp: str = None) -> WebClient.chat_postMessage:
        """
        ...
        if not response["ok"]:
            raise RuntimeError(f'Message failed to send with error: {response["error"]}')
    ...
    def add_reactions(self, timestamp: str, channel: str, reactions: List[str]) -> None:
        ...
        for react in reactions:
            ...
            if not response["ok"]:
                raise RuntimeError(f'Reaction failed to add with error: {response["error"]}')
